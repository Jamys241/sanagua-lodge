...
def _header_block(story, W, C, quote_label, quote_num, logo_b64=None):
    # Construir celda izquierda: logo (si existe) + nombre empresa
    logo_img = None
    if logo_b64:
        try:
...
def _build_quote_pdf(buffer, data, quote_num):
    ...
    _header_block(story, W, C, 'Cotización Sanagua Lodge', quote_num, logo_b64=data.get('logo'))
...
def _build_confirmation_pdf(buffer, data, quote_num):
    ...
    _header_block(story, W, C, 'Confirmación de Reserva', quote_num, logo_b64=data.get('logo'))
